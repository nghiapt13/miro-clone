...
import { Partcipants } from "./participants";
...
import { SelecionBox } from "./selection-box";
...
export const Canvas = ({
    ...
    const updateSelecionNet = useMutation((
        ...
        const ids = findIntersectingLayersWithRectangle(
            layerIds, layers, origin, current
        );
    ...
    }, [canvasState, resizeSelectedLayer, camera, translateSelectedLayers, continueDrawing, startMultiSelection, updateSelecionNet]);
    ...
    const layerIdsToColorSelecion = useMemo(() => {
        const layerIdsToColorSelecion: Record<string, string> = {};

        for (const user of selections) {
            const [connectionId, selection] = user;
            for (const layerId of selection) {
                layerIdsToColorSelecion[layerId] = connectionIdToColor(connectionId)
            }
        }
        return layerIdsToColorSelecion;
    }, [selections]);
                ...
                >
                    {layerIds.map((layerId) => (
                        <LayerPreview
                            ...
                            selectionColor={layerIdsToColorSelecion[layerId]}
                        />
                    ))}
                    <SelecionBox onResizeHandlePointerDown={onResizeHandlePointerDown} />
